...
var createPreprocesor = function(logger, config, basePath) {
  ...
  return function(content, file, done) {
    ...
    if(config.transformPath !== undefined){
      filePath = config.transformPath(filePath);
    }

    if (filePath === BRIDGE_FILE_PATH) {
      return done(content);
    }
    ...
    var output =
      '"test";window.__cjs_modules_root__ = "' + modulesRootPath + '";' +
      'window.__cjs_module__ = window.__cjs_module__ || {};' +
      'window.__cjs_module__["' + filePath + '"] = function(require, module, exports) {' +
      content + os.EOL +
      '}';
